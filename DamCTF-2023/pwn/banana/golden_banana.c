...
typedef struct _choice {
    char description[1024];       // Description of the choice
    location *location;           // Where to go when this choice is chosen
} choice;

typedef struct _location {
    char description[1024]; // Description of the location
    choice choices[4];  // List of choices
...
void load_game(game *g) {
    ...
    for (int i = 0; i < num_locations; ++i) {
        ...
        fgets(g->locations[index].description, 1024, game_file);
        ...
        for (int j = 0; j < g->locations[index].num_choices; ++j) {
            ...
            fgets(g->locations[index].choices[j].description, 1024, game_file);
        }
