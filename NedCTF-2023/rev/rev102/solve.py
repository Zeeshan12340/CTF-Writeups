...
def main(argv):
  ...
  def is_successful(state):
    ...
    # (!)
    return  b'Access Granted!' in stdout_output # :boolean
  ...
  def should_abort(state):
    stdout_output = state.posix.dumps(sys.stdout.fileno())
    return b'Access Denied!' in stdout_output  # :boolean
  ...
  if simulation.found:
    ...
    print(solution_state.posix.dumps(sys.stdout.fileno()).decode('latin1'))
    with open("payload","wb+") as w:
      w.write(solution_state.posix.dumps(sys.stdin.fileno()))
